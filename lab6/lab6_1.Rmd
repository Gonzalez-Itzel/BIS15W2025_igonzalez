...
```{r}
mammals <- read_csv("data/mammal_lifehistories_v2.csv") %>% clean_names() # cleans names in the files 
```
...
mammals %>% 
  ...
  mutate(gestiation_years=gestation/12) %>% 
  arrange(-gestiation_years)
```
...
```{r}
mammals %>% 
  mutate(across(everything(), tolower)) # mtate across all the varaibales and observations and change them to lowercase
```
...
mammals %>%
  mutate(across(-1, ~ tolower(.))) #apply tolower to all columns except the first
                                    # -1 means first column, "." <- do this to every cell 
# what it means: apply this function to every cell (.) except the first column (-1)
```
...
mammals %>% 
  select(genus, species, newborn) %>%
  mutate(newborn_new = ifelse(newborn == -999.00, NA, newborn))%>% # if new born is equal to -999.00 replace with NA, rename with new_born with the following info after ifelse 
  arrange(-newborn)
```
...
mammals %>% 
    ...
    mutate(max_life_new = ifelse(max_life == -999.00, NA, max_life)) %>% # third item in the ifelse is saying that if it does not equal -999.00 keep that number and mutate it to the new data
