...
```{r}
filter(fish, lakeid == "AL") # what you are searching for needs to be in qoutes
```
...
```{r}
filter(fish, lakeid != "AL") # excludes all the observations that are not AL
```
...
```{r}
filter(fish, length %in% c(167, 175)) # filters in fish, in the variable in length with values between 167 and 175
```


Alternatively, you can use `between` if you are looking for a range of specific values.  
```{r}
scale_length <- filter(fish, between(scalelength, 2.5, 3)) # between looks for a specific range
view(scale_length)
```

You can also extract observations "near" a certain value but you need to specify a tolerance.  
```{r}
filter(fish, near(radii_length_mm, 2, tol = 0.2)) # tolerance level filters on finding numbers near 2,1.8, and 2.2
```
...
```{r}
filter(fish, scalelength %in% c(2,4)) # operator looks for very specific values , there is no exact values of 2 and 4
```
