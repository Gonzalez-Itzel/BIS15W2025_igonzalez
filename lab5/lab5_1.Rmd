...
```{r}
filter(fish, lakeid =='AL' & length>350) # do two things at once
```

Notice that the `|` operator generates a different result. Why?
```{r}
filter(fish, lakeid =='AL' | length>350) # | is equivilant to "or", gives you observations that are AL or lengths greater 350 from any state
```
...
```{r}
mammals_mass <-select(mammals,family,mass)
bovidae_family <-filter(mammals_mass, family=='Bovidae', mass>450000)
bovidae_family
```
...
```{r}
new_mammals <- filter(mammals, family=='Felidae'& gestation<3)
new_mammals2 <- select(new_mammals, genus, family, gestation)
filter(new_mammals2, gestation!="-999")
```
