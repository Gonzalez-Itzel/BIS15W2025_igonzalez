...
It is not tidy because the income is spread throughout multiple variables. 

3. Use `pivot_longer()` to make the data tidy.  
```{r}
relig_income %>% 
  pivot_longer(-religion,# not moving religion
               names_to = 'income', # moving column names to a column named 'income'
               values_to = 'total') # move the values to a new variable called 'total'
```
...
billboard2 <- 
  billboard %>% 
  pivot_longer(wk1:wk76, # a range of columns that you want to move
               names_to = "week",
               values_to = "rank", 
...
This data is not tidy because the number of days are spread throughout on the horizontally.sequential column names.  
```{r}
plant_data
```
...
qpcr_untidy %>% 
  pivot_longer(
    ...
    names_sep="_", # seperated by an underscore the word experiment and replicate with the value 
