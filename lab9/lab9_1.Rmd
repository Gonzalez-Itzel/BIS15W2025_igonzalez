...
```{r}
datasets::USPersonalExpenditure # built in data
?USPersonalExpenditure

# column names are yeasrs
```
...
```{r}
expenditures <- USPersonalExpenditure %>% # changing it into a dataframe
...
expenditures <- expenditures %>% 
  pivot_longer(-expenditure,# not moving the expenditure variable
               names_to = 'year', # put the variables in a column called years
               values_to = 'cost') # put the values in a column called cost
```
...
```{r}
heartrate2 <- read_csv("data/heartrate2.csv") # contains two pieces of data in one variale
heartrate2
```
...
```{r}
heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = "_") %>% # seperate patient into two columns, variables are seperated by "-"
...
```{r}
heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = -1) #-1 removing the last character in the string
```
...
heartrate2 %>% 
  ...
  pivot_longer(-c(patient, sex),
               names_to = 'drug',
               values_to = 'dosage')
  # strngr dealing with strings(characters)
```
...
```{r}
heartrate3 %>% 
  unite(patient_sex, 'patient', 'sex', sep = '_') # does the opposite of seperate
```
...
pivot it back long back to its original form 
