...
hsprings <- data.frame(temp_C, scientist, spring, depth_ft)
hsprings
```
...
```
```{r}
summary(hot_springs) # be careful with the math 
```
```{r}
class(hot_springs$temp_C)
```
...
```{r}
setwd("/Users/itzel08/Desktop/BIS15W2025_igonzalez/lab2")
mountains <- read_csv("mountains.csv")
```
