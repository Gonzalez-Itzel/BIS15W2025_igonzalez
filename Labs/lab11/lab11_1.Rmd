...
```{r}
life_history %>% 
  ggplot(aes(x=order))+ # this is automatically doing the count for you
  geom_bar()+
  coord_flip()
```
...
life_history %>% 
  ...
  ggplot(aes(x=order, y=mean_mass))+ geom_col()+
  coord_flip()+
  scale_y_log10() # changes the scale
```
...
```{r}
life_history %>% 
  ggplot(aes(x=order, y=mass))+ # x is categorical and y is numerical 
  geom_boxplot(na.rm =T)+ # if its asking for a range use boxplot
  coord_flip()+
  scale_y_log10()
```
...
elephants %>% 
  ...
  geom_point()+
  geom_smooth(method = lm, se=F) # this adds the line of best fit, and se removes the grey line behind the blue line
```
