...
+`scale_fill_brewer()` is for fills  # bar plots
...
```{r}
my_palette <- paletteer_d("basetheme::brutal")
```
...
```{r}
my_palette <- paletteer_d("basetheme::clean")
...
```{r}
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass, fill=taxon))+ # facet used for categotical variables
  geom_boxplot()+
  facet_wrap(~trophic.guild, ncol=2, labeller=label_both)+ # ~(by)
  theme(axis.text.x = element_text(angle = 60, hjust=1),
        legend.position = "none",
        strip.text = element_text(size = 12, face = "bold")) # the grey strip with the text
```
...
homerange %>% 
  ...
  facet_grid(locomotion~.) # rows by column
```
```{r}
homerange %>% 
  ggplot(aes(x=log10.mass))+
