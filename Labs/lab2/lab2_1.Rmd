...
# can be used to change the class of an object
```
...
```
```{r}
anyNA(new_vector) # this is useful for determining if there are NA values that prevent calculations
```


4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm = TRUE) # na.rm will remove an NA
