...
```{r}
mean(large$sleep_total) # this is base R code, calculate the mean sleep total in the large data base
```
...
```{r}
msleep %>% 
  filter(bodywt>200) %>% # pull out the big ones
  summarize(mean_sleep_lg=mean(sleep_total)) # mean_sleeplg is the new variable, summarize()` will produce summary statistics for a given variable in a data frame
```
...
msleep %>% 
  filter(bodywt<10)%>% 
  summarize(mean_sleep_sm=mean(sleep_total)) # advantage for multiple variables you want to calculate 
```
...
msleep %>% 
  filter(bodywt>200) %>% 
  summarize(mean_sleep_lg=mean(sleep_total), # can do it all in one, summarizes data frame to our question and analysis
...
msleep %>% 
  ...
  summarize(across(everything(), mean, na.rm=TRUE)) # summarizes the mean across all numberic and observation variables 
```
```{r}
msleep %>% 
  select(where(is.numeric)) %>% 
  summarize_all(mean, na.rm=TRUE) # summarizes the mean across all numberic and observation variables 
```
...
msleep %>% 
  ...
  summarize(primate_mean_sleep=mean(bodywt),
  total=n())
...
msleep %>% 
  filter(bodywt > 100) %>% 
  summarize(n_genera=n_distinct(genus)) # number of obsevations in categorical 

```

```{r}
bodyweight_small <- msleep %>% 
  filter(bodywt > 100)

unique(bodyweight_small$genus) # this will give a list
```
...
```{r}
msleep %>% 
  count(vore) # counts the uniqueobservations in each varianle
```
...
```{r}
msleep %>% 
  tabyl(vore) # get some to go with it and gives you the NA
```
...
```{r}
msleep %>% 
  select(order, conservation) %>% # dont need to select, can also go straight to filter()
  filter(order=='Carnivora') %>% 
  count(conservation, sort = T) # sorts by descending order by count
```
