...
homerange %>% 
  ggplot(aes(log10.mass))+
  geom_histogram(color='pink3', fill='turquoise1') # make sure color name is in quotes
```
...
homerange %>% 
  ggplot(aes(log10.mass))+
  geom_density(fill='deepskyblue', color='black', alpha=0.5) #alpha reduces transparancy
```
...
```{r}
summary(homerange$log10.mass) # gives you quartile
```
...
```{r}
 homerange <- homerange %>% 
  mutate(mass_category=case_when(log10.mass<=1.7 ~ 'small', #case_when turn a continus thing into categorical
...
```{r}
homerange <- homerange %>% 
  mutate(mass_category=case_when(log10.hra<3.65 ~ 'very_small', #case_when turn a continus thing into categorical
                                log10.hra>= 3.65 & log10.mass <= 3.33 ~ 'medium',
                                log10.hra >3.33~ 'large'))
```
