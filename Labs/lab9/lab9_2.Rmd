...
+ `discrete` quantitative data that only contains integers, shoe size
+ `continuous` quantitative data that can take any numerical value
+ `categorical` qualitative data that can take on a limited number of values, color for examle
...
In short, **plot= data + geom_ + aesthetics**.  geom_type = type of data
...
```{r}
glimpse(iris)
# sepal.length is continous 
# species is catergorical
#
```
...
```{r}
ggplot(data = iris, #indicate data
...
```{r}
ggplot(data = iris, #indicate data
       mapping = aes(x=Species, y=Petal.Length)) + geom_boxplot() #map the aesthetics
```
...
```{r}
ggplot(data = iris,
       mapping = aes(x=Species, y= Petal.Length)) + geom_point()
```
...
ggplot(data= homerange,
       mapping = aes(x=log10.mass, y=log10.hra)) + geom_point() # overplotting
```
...
```{r}
ggplot(data= homerange,
       mapping = aes(x=log10.mass, y=log10.hra)) + geom_jitter() # adds some random noise to seperate some individuals points on the scatter plot
```

To add a regression (best of fit) line, we just add another layer.
```{r}
ggplot(data= homerange,
       mapping = aes(x=log10.mass, y=log10.hra)) + geom_point()+ geom_smooth(method=lm, se=T) # se removes the grey part
# this is specificying the linear model and whether or not are standard error shown 
```
...
```{r}
ggplot(data=homerange, 
       mapping = aes(x=log10.hra, y=log10.preymass)) + geom_point(na.rm = T) + geom_smooth(method = lm, se=T, na.rm=T) # removes the NAs
```
...
```{r}
homerange %>% 
  count(trophic.guild) # give readers a table first
```

Also notice that we can use pipes! The `mapping=` function is implied by `aes` and so is often left out. 
```{r}
homerange %>% 
  ggplot(mapping = aes(trophic.guild))+
  geom_bar() # use this when making counts of a categorical vairable
```
...
homerange %>% 
  ...
  ggplot(mapping = aes(x=common.name, y=log10.mass))+ # we dont have to use mapping, shortcut to aes only when using pipes
  geom_col()+
  coord_flip() # flips the bar graphs on their side
```
...
homerange2 %>%
  ...
  geom_col() # used for specifying x and y 
```
