...
```{r}
# cleaner, and tells you what class each variable belongs to
glimpse(fish)
```

`nrow()` gives the numbers of rows.
```{r}
# can ask how many rows
nrow(fish)
```

`ncol` gives the number of columns.
```{r}
# can how ask how many columns
ncol(fish)
```

`dim()` gives the dimensions.
```{r}
# give the dimensions of the data frame
dim(fish)
```
...
```{r}
# reads you the first n rows of the data frames
head(fish)
```

`tail()` prinst the last n rows of the data frame.
```{r}
# reads you the last n rows of the data frames
tail(fish)
```
...
```{r}
# gives number of observations for each of the factors/variables
table(fish$lakeid)
```
...
```{r}
# pulls out variables (columns) of interest
# first is the name of the data frame, variale, variable
