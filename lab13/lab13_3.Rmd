...
```{r}
register_stadiamaps("3d897520-176e-414a-8391-55907aad6c58", write = FALSE)
```
...
```{r}
lat <- c(34.67, 41.80) # from the summary above
long <- c(-124.1, -115.5) # from the summary above
...
```{r}
ggmap(map1) + 
  geom_point(data = spiders, aes(longitude, latitude), size=0.7) +
...
```{r}
shark_map <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom=5)
```
...
```{r}
fatal_attack_map <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom=5)
```

```{r}
ggmap(fatal_attack_map)
```

```{r}
ggmap(fatal_attack_map) + 
  geom_point(data = fatal_attacks, aes(longitude, latitude), size=0.9) +
  labs(x= "Longitude", y= "Latitude", title="Fatal Shark Attacks Locations")
```
