...
```{r}
inner_join(table_A, table_B, by="customer_ID") # only used for exact matches (example customer_ID)
```

`left_join`
Where did customer 4 go? Notice the missing data for customer 3.
```{r}
left_join(table_A, table_B, by="customer_ID") # matchig table A to table B
```

`right_join`
Where did customer 3 go? Notice the missing data for customer 4.
```{r}
right_join(table_A, table_B, by="customer_ID") # matching table A to table B
```

`full_join`
```{r}
full_join(table_A, table_B, by="customer_ID") # joins eveything
```

`anti_join`
Provides the rows in the first table for which there are not matching values in the second table.
```{r}
anti_join(table_A, table_B, by="customer_ID") # best for trouble shooting , finding observations that are problems 
```
...
```{r}
 spider_with_locs <- left_join(spider_records, spiders_locs, by="Accession")
```
...
```{r}
write.csv(spider_with_locs, file="spider_with_locs", row.names=F)
```
