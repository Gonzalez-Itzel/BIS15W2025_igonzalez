...
## Load the tidyverse
```{r}
library("tidyverse") # load this for things to work 
```
...
```{r}
data.frame(Sex, Length, Weight)
```


Here we combine our three vectors to create a data frame with the function `data.frame()`.
```{r}
hbirds <- data.frame(Sex, Length, Weight) # data.fram is way to make a data frame
```
...
```{r}
hbirds # dbl = numeric, chr = character
```
...
```{r}
names(hbirds) # gives you the column name
```
...
```{r}
str(hbirds) # tells you the structure
```
...
```{r}
hbirds <- tibble(sex=Sex, length=Length, weight_g=Weight) # makes a new data fram where it changes the Upper Case letters to Lower Case
...
The first row.  
```{r}
hbirds[1,] # what's in the 1st row
```
...
```{r}
mean(hbirds$length) # use $ to access variable in a data frame
```
...
```{r}
write.csv(hbirds, "hbirds_data.csv", row.names = FALSE) # allows us to save
                                                        # row.names = FALSE does not save the row names
```
